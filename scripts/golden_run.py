#!/usr/bin/env python3
"""Golden run: 296 (15m, $2k/20x/50%) vs 294 (15m, $1k/10x/100%). Monthly."""
...
CONFIGS = [
    ("bb_rsi_protected", "296 BB+RSI Protected", "15m", 2000, 20, 50),
    ("macd_atr_trend", "294 MACD+ATR", "15m", 1000, 10, 100),
]

MN = [("2025-12","2026-01"),("2026-01","2026-02"),("2026-02","2026-03"),
      ("2026-03","2026-04"),("2026-04","2026-05"),("2026-05","2026-06")]

for fname, label, iv, cap, lev, pct in CONFIGS:
    c = json.loads((STRATS / f"{fname}.json").read_text())
    c["basic"]["initial_capital"] = str(cap)
    c["basic"]["leverage"] = str(lev)
    c["basic"]["compound_profit"] = False
    for blk in ["long","short"]:
        for r in c.get(blk,{}).get("entry_rules",[]):
            r["position_pct"] = str(pct)
    
    print(f"\n{'='*65}")
    print(f"  {label}  {iv}  ${cap}  {lev}x  {pct}%")
    print(f"{'='*65}")
    print(f"  {'Month':<8s} {'Fills':>6s} {'PnL':>10s} {'Equity':>10s} {'Return':>8s} {'MaxDD':>8s}")
    print(f"  {'-'*55}")
    ...
    for ms, me in MN:
        ...
        pl = dict(strategy=c, rows_by_interval={iv:rows},
                  start=f"{ms}-01T00:00:00+00:00", end=f"{me}-01T00:00:00+00:00",
                  fee_rate="0.0004")
        ...
        total_pnl += p
        total_fills += f
        print(f"  {ms:<8s} {f:>6d} {p:>+10.1f} {e:>10.1f} {rt:>8s} {dd:>8s}")
    print(f"  {'-'*55}")
    print(f"  {'TOTAL':<8s} {int(total_fills):>6d} {total_pnl:>+10.1f}")
